...
import ToolBar from '../../../cong-trinh-KTSD-TNN/KTSD-phai-dang-ky/nuoc-bien/toolbar'
import CreateCTKTSDN_PDK_NuocBien from '../../../create-form/CreateCTKTSDN_PDK_NuocBien'

const CTKTSDN_PDK_NuocBien = () => {
  ...
  useEffect(() => {
    async function getDataCTKTSDN_PDK_NuocBien() {
      setLoading(true)
      await getData('CTKTSDN_PDK_NuocBien/danh-sach')
        .then(data => {
          setData(data)
        })
        .catch(error => {
  ...
  const columnsTable: TableColumn[] = [
    ...
    },
    {
      ...
      children: [
        {
          id: '#27',
          children: [{ id: '#27.1', label: '(27)', align: 'left' }]
        }
      ]
    },
    ...
    <Paper sx={{ p: 8 }}>
      ...
      <Grid className='_text_center'>
        <Typography className='font-weight-bold ' variant='h6'>
          CÔNG TRÌNH KHAI THÁC NƯỚC BIỂN THUỘC TRƯỜNG HỢP PHẢI ĐĂNG KÝ
        </Typography>
        <Typography className='font-weight-bold ' variant='h6'>
          (Kỳ báo cáo:{' '}
      ...
      {loading ? (
        ...
        <Grid className='_text_center' sx={{ mt: 3 }}>
          ...
          <TableComponent
            ...
            actions={(row: any) => (
              <Box>
                <CreateCTKTSDN_PDK_NuocBien isEdit={true} data={row} setPostSuccess={handlePostSuccess} />
                <DeleteData url={'CTKTSDN_PDK_NuocBien'} data={row} setPostSuccess={handlePostSuccess} />
              </Box>
            )}
          />
        </Grid>
...
export default CTKTSDN_PDK_NuocBien
